...
> **تحديث:** عولجت في هذا الفرع كل الملاحظات الحرجة وعالية الخطورة تقريباً عبر سلسلة الالتزامات المرفقة، عدا **C-1 (ملفات Gradle)** التي تتطلب إعادة توليد عبر `npx cap sync android` من بيئة بها Android SDK. راجع قسم **12. حالة المعالجة** في نهاية التقرير.
...
|---|---|---|---|
| C-1 | ملفات Gradle مفقودة | ⏳ **متبقية** — تتطلب Android SDK | — |
...
استعادة مشروع Gradle تتطلب تشغيل الأمر التالي في بيئة مثبَّت بها Android SDK:

```bash
npx cap sync android
```

ثم ضبط `applicationId` و `versionCode`/`versionName` و `minSdkVersion 23`
وإعدادات التوقيع عبر متغيرات البيئة (لا تودع كلمات المرور في المستودع) —
راجع المقتطف في قسم C-1 أعلاه.
