...
**خلاصة التوجيه كاملاً (6 دفعات، من `23eb73e` إلى هنا):** من 1152 اختباراً × 121 ملفاً عند التمكين إلى **1302 اختباراً × 131 ملفاً** عند الإغلاق (+150 اختباراً) · الراتشيت من 64.1 إلى **66.7/61.5/50.1** (تسع رفعات، لم يهبط قط) · **8 عيوب حقيقية** سقطت في الطريق (NotificationType، أسود الخط العربي، aria-hidden، سباق مؤقت الإغلاق، تبعية loadCards الدائرية، قيد السرعة باتجاه الحافة، الاستيراد الجشع لـchart.js، خط Inter الميت) · صفر انحدار سلوكي مقيساً في كل دفعة (التوصيف أولاً، ثم البقاء أخضر عبر إعادة البناء).

- **سلم التغطية — خدمة السحابة (العلة التاسعة الحقيقية) — مكتمل ومسلّم، v23.3.4**:
  درجة القاع في السلم كانت `cloud.ts` — خدمة المزامنة السحابية المشفّرة E2E (Supabase) — عند **3.44%** تغطية. جلستها 23 اختباراً بتشفير **حقيقي** (PBKDF2 600k → AES-GCM) وقاعدة بيانات **حقيقية**، والموكات عند حواف المنصة فقط (fetch وsecureStore):
  - **المغلف المشفر**: ذهاب-وإياب أمين، شكل الغلاف (ملح 16B، iv 12B، v:1)، عشوائية الملح بين تشفيرين، وكلمة السر الخاطئة تُرفض بسلامة AES-GCM.
  - **دورة الجلسة**: تسجيل دخول يحفظ ويحوّل الانتهاء من ثوانٍ إلى ميلي، الجلسة المنتهية تُمحى وتقرأ كعدم، إعادة المحاولة في sbFetch (استرجاع بعد 5xx على المحاولة الثانية، استسلام بعد 3 بمؤقتات مزيفة للتراجع الأسي)، وفشل التحديث/الخروج يمسح الجلسة محلياً دائماً.
  - **النسخ الاحتياطي**: بلا جلسة أو بلا user_id يُرفض؛ وفي المسار السعيد **يفكّ الاختبار ما رُفع فعلاً**: جسم POST يُفك تشفيره بكلمة السر ويعود بالصفوف كما دخلت، مع record_counts وlastCloudBackup.
  - **الاستعادة**: no_backup_found، wrong_password كرسالة واضحة لا كانهيار تشفير، والمسار السعيد يصفّر الصفوف القديمة ويعيد الاستيراد ويستعيد الإعدادات.
  - **🐞 العلة التاسعة الحقيقية**: اختبار «الصف القديم يجب ألا ينجو» فشل — التحقيق كشف أن مرحلة التصفير في `cloudRestore` تستدعي `delFunc(id)` **منفصلة عن مستقبلها** فيكون `this` غير معرف داخل deleteTransaction، والـTypeError يبتلعه `.catch(silentFail)` الموضوع أصلاً لتسجيل أخطاء الحذف — **فالاستعادة السحابية لم تكن تصفّر شيئاً قط في الإنتاج**، بل تعيد الاستيراد فوق البيانات القديمة (تكرارات وسجلات بُعثت من قبرها). الإصلاح سطر واحد: `delFunc.call(dbRecord, id)` + تعليق يوثّق القصة. الدليل على إصلاحها: الاختبار الذي اصطادها أصبح أخضر.
  - **الأرقام**: cloud.ts **3.44% → 94.48%** أسطراً · **1376/1376 × 139** (+23) · التغطية 68.13/62.8/51.34 → **الرفعة الثانية عشرة 68.1/62.7/51.3** · ci:check exit 0 · الإصدار `23.3.4`.

- **استمرار التفكيك (TravelBudget — آخر المونوليثات الكبرى) — مكتمل ومسلّم، v23.3.3**:
  المسح بعد دفعة التفكيك كشف أن ميزانية السفر (584 سطراً) أكبر من ملفين مما فُكك قبله، وتوصيفها كُتب أصلاً «قبل الاستخراج». سقطت بنفس العقد — الاختبارات القائمة (12) خضراء قبل وبعد:
  - **TravelBudget: 584 → 194 سطراً**: البطاقة (TripCard، 277 سطراً حرفياً) دخلت برنامج cv-item/contain-card، **حساب تحويل العملات** — جوهر الشاشة الذي يثبته التوصيف — أصبح نقياً في `tripSpendMetrics` (الحد الأجنبي × السعر، والمصروف ÷ السعر، مع حواجز المعدل الصفري والقيمة غير القابلة للتحليل)، مفردات الطقس (`extractCountry` ثنائية اللغة + `getWeatherInfo` بمدارات Open-Meteo) في `travelWeather`، وتأثير التوقّع الحي كاملاً (ذاكرة أولاً، ثم fallback محلي مضاد للوميض، ثم geocoding + طقس مع إجهاض عند تغيير الرحلة) في hook مستقل `useTravelForecast`.
  - **+8 اختبارات بدائيات** (اثنان منها صحّحا افتراضاتي أنا أثناء التوصيف: حارس المعدل يُطبَّق على اتجاه الحد أيضاً، والكود 99 يقع في نطاق العواصف لا «المعتدل») + توسيع pin الأداء لـTripCard.
  - **الأرقام**: **1353/1353 × 138** · التغطية 67.2/62.12/50.93 → **الرفعة الحادية عشرة 67.2/62.1/50.9** · ci:check exit 0 · الإصدار `23.3.3`.

- **دفعة التفكيك المؤجل (الدين الهيكلي الأخير) — مكتملة ومسلّمة، v23.3.2**:
  الملفات الثلاثة المؤجلة سقطت بنفس منهجية التوجيه — توصيف أولاً (17 اختباراً كُتبت خضراء على المونوليث قبل أي تغيير)، ثم التفكيك، ثم الاختبارات نفسها خضراء على البناء الجديد:
  - **Assets.tsx: 591 → 200 سطراً**: البطاقة (AssetCard)، لوحة الملخص الزجاجية (AssetSummaryPanel)، فلاتر الفئات (AssetCategoryFilter)، تجميع المحفظة النقي (assetPortfolio)، وورقة أنماط الطباعة (assetsPrintCss) — كلها وحدات مستقلة. **البطاقة المستخرجة دخلت برنامج cv-item/contain-card** وفق قاعدة §5.1-3.
  - **الأنبوب المكرر قُتل**: أنبوب تصدير PDF (~150 سطراً: بروكسي oklch→rgb، تجريد الـbackdrop-filter، فرض عرض سطح المكتب، تقسيم صفحات A4) كان يعيش **نسختين متطابقتين** في Assets وAssetDetailModal — الآن خدمة واحدة `exportElementAsPdf` يمررها كلٌّ منهما بمعاملاته (فروق الاسم والخلفية الداكنة محفوظة عمداً). اختبارات Batch 5 لأنبوب PDF ظلت خضراء عبر الاستبدال — الدليل السلوكي على التطابق.
  - **AssetDetailModal: 472 → 329 سطراً** (الأنبوب استُبدل بنداء خدمة).
  - **Investments.tsx: 505 → 213 سطراً**: النافذة (InvestmentModal، 232 سطراً حرفياً)، البطاقة (InvestmentCard مع cv-item)، وحسابات الربح النقية (investmentMetrics). **ملاحظة سلوكية موثقة**: زر الحذف في إجراءات hover لسطح المكتب يحذف مباشرة بلا ورقة تأكيد (بخلاف باقي مسارات الحذف) — سلوك قائم حُفظ كما هو لأن التفكيك لا يغيّر السلوك؛ مرشح لقرار المالك لاحقاً.
  - **+27 اختباراً**: 17 توصيفاً (شاشتا الأصول والاستثمارات كاملتين: الملخص، الفلترة، الشارات، التحديد الجماعي، البوابة، الإضافة/التعديل/الحذف) + 8 للبدائيات النقية (حافة قسمة الصفر، NaN، المحفظة الفارغة) + توسيع pin الأداء للبطاقتين المستخرجتين.
  - **الأرقام**: **1344/1344 × 137** · التغطية 67.13/62.08/50.64 → **الرفعة العاشرة 67.1/62.0/50.6** · ci:check exit 0 · الإصدار `23.3.2`.

- **ملحق الإغلاق (قواعد §5.1 الأخيرة من المحور الرابع) — مكتمل ومسلّم، v23.3.1**:
  آخر ما تبقى من المقترح المعتمد ولم يُسلَّم: قواعد أداء 60FPS الثلاث الباقية، كلها الآن مثبتة باختبارات سكونية في CI بدل مراجعة PR:
  - **القاعدة 3 (الشاشات الطويلة + احتواء البطاقات)**: أدوات جديدة `cv-item`/`cv-item-lg` في index.css (`content-visibility: auto` + `contain-intrinsic-size: auto` بذاكرة الارتفاع الحقيقي بعد أول رسم) على **ست قوائم طويلة**: سطر المعاملة (TransactionItem)، المتكررة (نوعان: المجدولة والمكتشفة)، سجل التدقيق، بطاقات الأقساط، وبطاقات المسرد — فالمتصفح يتخطى تخطيط ورسم كل صف خارج الشاشة بالكامل. والبطاقات المتدفقة بلا زخارف خارج الحدود تنال `contain-card` (`contain: layout paint`). المسرد أخذ `cv-item-lg` فقط (بلا contain) حفاظاً على تحويلات القلب 3D.
  - **القاعدة 2 (ميزانية الـblur)**: خمسة مواضع كان **العنصر المتكرر نفسه** سطحاً زجاجياً — قائمة من 20 عنصراً = 20 طبقة backdrop-filter حية، وهذا بالضبط ما وُضع سقف ≤3 لمنعه. حُذف الـblur من عناصر: المتكررة ×2، سجل التدقيق، الأقساط، وقواعد التصنيف التلقائي. خلفياتها شبه معتمة فوق خلفية صفحة شبه مسطحة فالفروق البصري صفر تقريباً — وعينك البوابة كالعادة. الكاروسيل المقيد في العملات (~6 بطاقات، 1–2 مرئية) بقي ضمن الميزانية كأسطح مرئية.
  - **القاعدة 5 (will-change بوصفة)**: النسخة القائمة الوحيدة (`.card-stable` في ملفي CSS) حُذفت — `translateZ(0)` يمنح الطبقة أصلاً، وwill-change الدائم طبقة لا تسترجعها الـGPU. **الوصفة الحالية: صفر will-change في كل src**، واختبار حارس يمسح الشجرة كاملة ويفشل أي إضافة مستقبلية بلا استثناء موثق.
  - **15 اختباراً جديداً** في `tests/unit/perfRules.test.ts`: تثبيت الأدوات الثلاث في index.css، وجود cv-item في كل قائمة، منع الزجاج عن أي عنصر مُرقمن، وصفر will-change.
  - **الأرقام**: **1317/1317 × 134** (+15) · التغطية 66.95/61.67/50.31 فوق السقاطة 66.7/61.5/50.1 (لم تُمسّ — الملحق لا يضيف تغطية src بل يحرس القواعد) · ci:check exit 0 كاملة · بناء 10.08 ثانية · الإصدار `23.3.1` في constants وpackage.json.

- **الدفعة 5 (الاتساق الشامل + الصقل) — مكتملة ومسلّمة**:
  - **ثلاثي السلم الأخير سقط**: NavigationDrawer من 17.4% إلى **100%** (6 اختبارات: البنية، إبراز المسار، التنقل+الإغلاق، الخلفية، خارج الشاشة، الإصدار)، DevUnlockModal من 9.61% إلى **92.6%** (7 اختبارات منها مسار PBKDF2 كامل حقيقي 600k تكرار ومقارنة ثابتة الزمن)، AssetDetailModal من 13.39% إلى **71.3%** (طباعة الويب + أنبوب PDF كامل بموكات jspdf/html2canvas + الحاوية المخصصة للطباعة).
  - **توكنات الـease الأربعة** في `:root` (`--ease-snappy/smooth/gentle/playful`) مشتقة من SPRINGS — **اختبار سكوني يثبّتها مساويةً لـspringCss() فلا تنحرف أبداً** — فتشارك Tailwind (ease-[var(--ease-snappy)]) والأنماط السطرية مصدراً واحداً.
  - **مسح bezier اليدوي حتى النهاية**: شاشة البدء، قلب البطاقة، بطاقة المسرد، مؤشر المقسّم في الإعدادات، وثلاثة مواضع في صفحة الإضافة — كلها على الزنابك المعتمدة. **وCSS ميت حُذف**: `.deck-container`/`.deck-card-wrap` ماتا مع إعادة بناء الدفعة 3 وبقيا في CARD_CSS حتى هذا المسح.
  - **الدرج الجانبي**: انزلاق على الزنبرك الأملس (`--ease-smooth`) واختيار وجهة يجيب بـtouch.select.
  - **الإصدار**: `23.3.0` في constants وpackage.json (اختبار المواءمة يحرس).
  - **الأرقام**: **1302/1302 × 131** (+21) · التغطية 66.94 ثم 66.98 في تشغيلين كاملين → **الرفعة التاسعة والأخيرة 66.7/61.5/50.1** · ci:check exit 0 كاملة · بناء 9.4 ثانية.

**بانتظارك للتوقيع النهائي**: بناء `@abc` على جهازك لإصدار **v23.3.4** الموقّع (يشمل الملحقات الأربعة — وعلة الاستعادة السحابية مصلحة فيه) — ثم يُغلق التوجيه رسمياً.

- **التوجيه 19 الدفعة 4 (الرسوم البيانية والتقارير — حيث تُقرأ القصة المالية) — مكتملة ومسلّمة**: شبكة رسم موحّدة الدلالة لأربعة أسطح، بنفس المنهجية:
  - **وحدة الثيم الموحدة `src/core/chartTheme.ts` (تغطية 100%)**: الزوج الدلالي (دخل أخضر زمردي/مصروف وردي — قرينَي توكنَي --color-income/--color-expense)، بطاقة تلميح زجاجية داكنة واحدة، شبكة باهتة يتحدث فيها خط الصفر وحده، صوت محاور واحد بخط التطبيق، تدرجات عمودية ناعمة، وبوابة حركة (400ms، ولا شيء إطلاقاً مع تفضيل تقليل الحركة).
  - **التوصيف أولاً**: 12 اختباراً ثبّتت عقود البيانات للأسطح الأربعة قبل التوحيد (ترتيب المتتاليات وقيمها، قطع الدونات 72%/70%، تجميع الاستثمارات بالجمع، حالات الفراغ) — عبر مُحمِّل مزيف يلتقط كل تهيئة تُسلَّم لـchart.js.
  - **ما وُحِّد**: ثلاثة سوداء مختلفة للتلميح → بطاقة زجاجية واحدة؛ أخضران للدخل → زوج واحد؛ حشوات مسطحة → تدرجات ناعمة؛ bezier يدوي 750ms → بوابة 400ms الموحّدة.
  - **عيبان حقيقيان (سادس وسابع) سقطا في الطريق**: (1) NetWorthTrend كان **يستورد chart.js بجشع ويسجّل متحكماته على مستوى الوحدة** — يُعطِّل المُحمِّل الكسول المشترك ويحمّل المكتبة مع حزمة لوحة التحكم؛ رُحِّل إلى getChart() كسلفه الثلاثة. (2) PortfolioBreakdown كان يطلب خط **Inter في وسيلة إيضاحه — عائلة حُذفت من الحزمة في الدفعة 1** فكان يسقط صامتاً إلى الاحتياطي؛ الآن يطلب خط التطبيق بالاسم.
  - **اختبار الحقيقة الواحدة المنقلبة**: اختبار التوصيف ثبّت الجشع (getChart لا يُستدعى أبداً) — الإصلاح المُصرَّح به قلب الحقيقة، فاستُبدل الاختبار بعقده الجديد: الكسل + التلوين بالإشارة + تدرج الصفر + الزجاج والشبكة.
  - **الأرقام**: **1281/1281 × 130** (+32: وحدة الثيم 14 + الأسطح 18) · NetWorthTrend من 25.5% إلى 90.4% سطور · التغطية 65.92 ثم 65.97 → **الرفعة الثامنة للراتشيت إلى 65.7/61.0/49.9** · ci:check exit 0 كاملة (بناء 9.5 ثانية).
  - **التالي**: فحصك البصري على جهازك (تلميح واحد زجاجي في كل الرسوم، التدرج الناعم في تقرير الاتجاه، تدرج الصفر الواعي في صافي الثروة)، ثم الدفعة 5 (الاتساق الشامل للشاشات الثانوية + الصقل + إغلاق التوجيه بإصدار v23.3.0).

- **التوجيه 19 الدفعة 3 (البطاقات والمحافظ — تجربة Revolut) — مكتملة ومسلّمة**: تفكيك الـ553 سطراً + سلسلة السحب + المفردة اللمسية، بنفس المنهجية:
  - **التوصيف أولاً + توسيعه**: 13 اختباراً قائماً + **7 جديدة** ثبّتت المسارات العمياء (الحذف ×3 بما فيه الإلغاء، نقاط التنقل، إخفاء الرقم، تفعيل لوحة المفاتيح، فشل التحميل) — **20/20 بقيت خضراء عبر التفكيك كله**.
  - **التفكيك**: 553 سطراً → جذر تركيب 176 سطراً يملك الحالة ودورة البيانات فقط + 6 قطع ≤139 سطراً في `deck/` (الرأس، حالة الفراغ، الرزمة، شريط التلصيص، لوحة التفاصيل، التذييل) + خطافان (`useCardText`، `useDeckSwipe`). الجذر من 79% تغطية إلى **100%**.
  - **سلسلة السحب**: اسحب البطاقة النشطة جانبياً للتنقل في الرزمة — واعٍ بالـRTL (يمين = التالي في العربية، يسار = التالي في الإنجليزية)، حواف الرزمة تقاوم (الاتجاه بلا بطاقات لا يتحرك ولا يُنفَّذ ولو بفحشة)، مطاطية بعد 96px، التزام بعد 48px أو فحشة 0.5px/ms، ارتداد زنبركي 320ms، وتفضيل تقليل الحركة يوقف التتبع الحي ويُبقي الأمر.
  - **دخول البطاقة على الزنبرك الهادئ**: كل تبديل بطاقة يعيد تشغيل دخول `deck-active-enter` (gentle 320ms، translateY+scale، تحويل/شفافية فقط — عقد 60FPS)، والشرائط والنقاط انتقلت من bezier يدوي إلى توكين `springCss('snappy')`.
  - **المفردة اللمسية للمحفظة**: touch.select عند اختيار بطاقة (شريط/نقطة/سحبة)، touch.light عند إظهار الرقم، touch.destruct عند حذف مؤكد، touch.confirm عند حفظ/تحديث بطاقة.
  - **عيب حقيقي خامس، جذري**: `loadCards` كان يعتمد على `activeCardId` → **كل تبديل بطاقة يعيد استعلام IndexedDB كاملاً ويومّض مؤشر التحميل فوق الرزمة**، والتركيب الأولي كان يحمّل مرتين (سباق يفكّل المعالجات في منتصف إيماءة). أُصلح بقرار الاختيار التلقائي عبر الصيغة الدالية للمُعطِي — التبديل صار حالة خالصة بلا أي استعلام.
  - **إصلاحان إضافيان**: قيد السرعة في useDeckSwipe لم يكن يُقيَّد باتجاه الحافة (فحشة نحو حائط كانت تستدعي onNext) — أُقيد؛ وزر الإظهار كان يحمل aria-label لـ«رجوع» (أحفورة نسخ) — سُمّي باسم فعله.
  - **الأرقام**: **1249/1249 × 129** (+24) · التغطية 65.19 ثم 65.22 في تشغيلين كاملين → **الرفعة السابعة للراتشيت إلى 65.0/60.0/49.3** · ci:check exit 0 كاملة.
  - **التالي**: فحصك اللمسي على جهازك (اسحب البطاقة يميناً للبطاقة التالية، ولاحظ زنبرك الدخول والاهتزازات)، ثم الدفعة 4 (الرسوم البيانية والتقارير — شبكة رسم موحدة الدلالة + تلميحات زجاجية).

- **التوجيه 19 الدفعة 2 (ورقة الإضافة السريعة — أولوية الإبهام) — مكتملة ومسلّمة**: بموجب التفويض السريع، وبالمنهجية ذاتها — التوصيف قبل البناء:
  - **التوصيف أولاً**: 16 اختباراً ثبّتت عقد QuickAddModal كاملاً قبل لمس أي سطر (عقد dialog/aria-pressed، تعقيم المبلغ، رفض الفراغ/الحسابات، الحفظ الافتراضي للفئة «طعام» + الإنجاز FIRST_TRANSACTION، منع السنة المقفلة، وضعي التحرير والحذف، بوابة التيربو بقفلها وفتحها، تحليل SMS نجاحاً وفشلاً، إغلاق الخلفية). **بقيت خضراء عبر البناء كله**.
  - **الورقة السفلية القابلة للسحب**: هوك `useSheetDrag` جديد — مقبض يسحب الورقة: إقالة بعد 96px أو رفرفة أسرع من 0.6px/ms، نطاق مطاطي بعد 120px بنصف المعدل، ارتداد زنبركي 320ms بمنحنى springCss('smooth')، وتفضيل تقليل الحركة يوقف التتبع الحي ويُبقي الإقالة (6 اختبارات فيزيائية مستقلة). حافة الورقة انتقلت إلى `var(--radius-sheet)`.
  - **المفردة اللمسية في التدفق المالي**: touch.error لكل رفض تحقق وللسنة المقفلة، touch.confirm للحفظ والتحديث، touch.destruct للحذف المؤكد، touch.select لتبديل مصروف/دخل، touch.light لاختيار الفئة ولنجاح تحليل الرسالة (8 اختبارات لمسية).
  - **ترحيل useHaptic القديم**: أصبح توجيهاً رقيقاً إلى المفردة الموحدة (نفس الواجهة والأسماء؛ light→light, medium→select, heavy→destruct, success→confirm, error→error, selection→light) — SwipeableRow ورث البوابات الثلاث (الإعداد + تقليل الحركة + never-throws) دون تعديل سطر واحد فيه.
  - **عيب حقيقي مكتشف ومصلح**: مؤقت إغلاق الورقة (300ms) كان يمسح حالة جلسة جديدة إذا أُعيد فتح الورقة أثناء انتظار أنيميشن الإغلاق — سباق مؤقتات يعقّب تحريراً فورياً بعد إغلاق؛ أُضيف حارس يتحقق أن الورقة ما زالت مغلقة قبل المسح.
  - **الأرقام**: **1225/1225 × 128** (+32) · التغطية 64.93 ثم 64.91 في تشغيلين كاملين → **الرفعة السادسة للراتشيت إلى 64.8/59.8/49.2** · ci:check exit 0 كاملة (tsc أولاً).
  - **التالي**: فحصك البصري للورقة على جهازك (اسحب المقبض للأسفل — الإقالة بالمسافة والرفشة، والمطاطية بعد 120px)، ثم الدفعة 3 من سلم المتبقي (lockModal 9.61، detailModal 13.39، drawer 17.39).

- **التوجيه 19 الدفعة 1 (تحويل لوحة التحكم) — مكتملة ومسلّمة**: انطلاقاً من تفويض `23eb73e` (تجربة الدفعة 0 موثّقة على ويندوز/JDK21 + تجميع الخط معتمد + تفويض سرعة قصوى):
  - **الخط الفعلي مجمّع ولأول مرة بالعربية**: IBM Plex Sans Arabic (5 أوزان × عربي+لاتيني ≈ 316KB woff2 في الحزمة) — **مع إصلاح عيب تاريخي**: `fontOptimizerPlugin` كان سيحذف مجموعة اللغة العربية من خط عربي (تعليقات fontsource تلتصق بذيل الكتلة السابقة عند التقسيم)؛ أعيد كتابته كقائمة بيضاء موجبة على اسم الملف. حُذفت Tajawal×6 وManrope×3 وInter×3 من الكود والتبعيات.
  - **توصيف قبل الهدم (15 اختباراً)**: سلوك ClassicDashboard كجذر تكوين موثّق بالكامل (homeOrder، وضع التحرير، الوضع البسيط، التوائم، البانر، الأخطاء، الهياكل العظمية) — **وبقي 15/15 أخضر بعد إعادة البناء كاملة**.
  - **التفكيك**: ClassicDashboard من 446 سطراً إلى جذر تكوين ~200 + قطع أحادية المسؤولية: `SmartActionBanner`، `DailyTipCard`، `EditableSection`، `sections.tsx` (جدول توجيه)، **`HeroBalanceCard`** (زجاج قوي + هالة دلالية زمردي/مرجاني حسب إشارة صافي الثروة + NumberFlow + عقد a11y الموروث كاملاً)، **`PulseStrip`** (شريط واحد موحد ~64px يوارث ~300px من بطاقات السوق الأربع المتكدسة، بنفس عقود البيانات والfallback). حُذف BalanceCard وMarketWidgets القديمان.
  - **`NumberFlow`**: العدّاد التكيفي (rAF بطابع الزمن الخاص بالإطار، مدة تكيفية 200-800ms بحسب حجم التغير، reduced-motion = قفزة فورية، أرقام ثابتة العرض بلا رقصة أفقية) + إصلاح إسقاط `aria-hidden` وقت التشغيل.
  - **عيب حقيقي مكتشف ومصلح من الدفعة 0**: `@capacitor/haptics` v8 يصدّر `NotificationType` لا `NotificationStyle` — مرّ عبر بوابة الإصدار لأن `ci:check` لم تكن تتضمن tsc؛ **أُضيف `tsc --noEmit` في مقدمة ci:check** فلا يهرب خطأ نوع ثانٍ.
  - **الأرقام**: **1193/1193 × 125** (+41) · تغطية مقيسة 64.45-64.55 → سقاطة رفعة خامسة **64.4/59.6/48.6** (بأمان انحراف) · `ci:check` exit 0 كاملة (الآن 6 مراحل تبدأ بـtsc) · 0 ثغرات · بناء 10.1s.
  - **التالي**: فحصك البصري للوحة الجديدة (الوضع الليلي + النهاري)، ثم الدفعة 2 (ورقة الإضافة السريعة السفلية + دمج المفردات اللمسية) — مفوَّضة ومجهزة للانطلاق فوراً.

- **التوجيه 19 (ثورة الـ UI/UX الشاملة) — الدفعة 0 «التمكين» مكتملة، البوابة البصرية بانتظار فحص المالك**:
  - **نظام الرموز معتمداً ومنصّتاً**: سلّم **Deep Obsidian** الكامل (`#070A10→#243146`، 6 طبقات) في `@theme`؛ الوضع الليلي الافتراضي ولوحة `dim` أُعيد تأسيسهما عليه (الخلفية `#0B0F17` بالضبط)؛ الزوج المالي **زمردي/مرجاني** `#34D399/#FB7185` الآمن لعمى الألوان؛ `primary-night #7CB0FF`؛ سلّم حواف دلالي 5 درجات (`chip/tile/card/sheet` بأسماء لا تصطدم بـTailwind فلا يتغير أي حجم قائم)؛ سلّم ارتفاع `e1..e4`.
  - **توحيد الخطوط**: IBM Plex Sans Arabic عائلةً وحيدة عبر CSS/HTML/PDF/الرسوم/بيانات اللغات — **اكتشاف مُفصح**: لا يوجد ملف خط في المشروع أصلاً (التطبيق كان يرسم بخطوط النظام دوماً)؛ تجميع ملف الخط الفعلي مقترح للدفعة 1 خلف بوابتك البصرية.
  - **البدائيات الأربع + طبقة اللمس، مختبرة بالكامل**: `<NumberText/>` (tabular-nums + حارس NaN)، `<GlassPanel/>` (وصفة الزجاج الموحدة)، `motion/tokens.ts` (4 ينبوعات + مقرّب cubic-bezier + بوابة reduced-motion)، `core/haptics.ts` (مفردات light/select/confirm/destruct/error/triumph — native عبر @capacitor/haptics وweb عبر navigator.vibrate، محترمة لمفتاح الإعدادات الجديد `hapticsEnabled`).
  - **إعداد جديد ظاهر**: مفتاح «الاهتزاز اللمسي» في الإعدادات العامة (مترجم بـ11 لغة) + تسجيله في فهرس البحث.
  - **الأرقام**: **1152/1152 × 121** (+40 اختباراً) · تغطية مقيسة **64.27/59.25/48.48** فوق سقاطة مرفوعة رفعة رابعة إلى **64.1/59.2/48.3** · `ci:check` exit 0 كاملة · 0 ثغرات.
  - **الخطوة التالية**: فحصك البصري على جهازك (الوضع الليلي تغيّر فعلياً إلى Obsidian — هذا هو التغيير المرئي الوحيد المتعمد)، ثم **الدفعة 1: لوحة التحكم الرئيسية**.
