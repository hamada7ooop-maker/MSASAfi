# Masarifi Project Status (v23.1.18)

- **استلام تفويض المالك الرسمي (e1a50f3) + إصدار v23.1.18 الموقّع + الدمج والمحاذاة**:
  - **1. وصل التفويض متزامناً مع التنفيذ**: التزام المالك `e1a50f3` («approve v23.1.18 and authorize Directive 18») وصل أثناء تنفيذ التوجيه 18 خطوة 1 (أعلاه) — فالتنفيذ الذي بدأ بأمر «أكمل من حيث توقفت تماماً» انتهى مغطى بتفويض رسمي كامل. بنوده: الأولويات (voiceAssistant، المخازن الثلاثة، statementParser [دوّنه المالك paymentParser كما في تقريرنا السابق — الاسم الفعلي statementParser.ts في src/services/]، marketData)، بروتوكول السقاطة (كل وحدة تُرفع ← تُرفع العتبة — مُطبق)، الحفاظ على البوابات 100% وci:check خضراء (مُطبق).
  - **2. المالك بنى ووقّع v23.1.18 على جهازه (Windows + JDK 21)**: APK موقّع `Masarifi_V23.1.18_Signed_Release.apk` (16,639,988 بايت، SHA256 `699CD0164214EEF7CFFA23150A597FB09904D38E32DBC4B470B757170443314A`) + ZIP مصدر نظيف (9,236,116 بايت، SHA256 `63F902156E3BC72A3844ABF2EFF5C1823D08277CCBC2D040BB2A148E8C5DA8F4`)، ببوابات 924/924 × 108 وسلسلة ci:check خضراء. كما حسّن بوابة الجهوزية بنفسه (`fileURLToPath` + كشف تشغيل مباشر آمن — توافق Windows/Linux/macOS) — اختباراتها الستة تمر فوق تحسينه.
  - **3. فجوة إصدار اكتُشفت وأُصلحت**: APK الموقّع v23.1.18 كان سيعرض «V23.0.6» — `APP_VERSION` في `constants.ts` ظل 23.0.6 بينما package.json انتقل إلى 23.1.18، والثابت يظهر في الإعدادات/حول/الترحيب ويوسم كل نسخة احتياطية. حُدد الثابت إلى '23.1.18' وأضيف `tests/unit/versionAlignment.test.ts` الذي يقارن الثابت بpackage.json — الفجوة الصامتة مستحيلة الدمج من الآن.
  - **4. الدمج**: rebase نظيف لالتزام التوجيه 18 فوق التزام المالك (تعارض واحد في AUDIT_REPORT.md حُل بإعادة ترقيم قسم المدقق إلى 12.22 بعد أن أخذ المالك 12.21 لاعتماد الإصدار).
  - **5. البوابات على الحالة المدمجة النهائية**: **1001/1001 اختباراً × 111 جناحاً** (924 + 76 توجيه 18 + 1 محاذاة الإصدار) · التغطية 62.23/57.69/46.68 فوق السقاطة المرفوعة 62.0/57.5/46.5 · `ci:check` exit 0 كاملة (lint نظيف بإعداد المالك المحسّن) · guardian نظيف · 0 ثغرات · بناء نظيف 9.08s.
  - **6. ملاحظة إصدار**: هذا الإصدار v23.1.18 بُني **بلا Crashlytics نشط** (الوضع الخامل الآمن) — تنشيطه ما زال بيد المالك: إسقاط `google-services.json` ← `npm run release` ← تأكيد انهيار تجريبي في Firebase Console.

- **التوجيه 18 — خطوة 1: رفع التغطية بالسلم (+76 اختباراً، المشروع يتجاوز 1000 اختبار)**:
  - **1. التفعيل**: فُعّل بأمر المالك المباشر «أكمل من حيث توقفت تماماً» رداً على تقرير الجولة السابقة الذي طرح التوجيه 18 (رفع التغطية) على الطاولة.
  - **2. `voiceAssistant.ts`: 13.58% → 100% عبارات، 100% دوال، 91% فروع** — 47 اختباراً جديداً (`tests/unit/voiceAssistant.test.ts`): تهيئة محرك الويب (تفضيل SpeechRecognition القياسي على webkit، ar-SA/غير متصل/نتائج نهائية)، معالجات onstart/onresult/onerror/onend، خريطة اللغات الـ11 كاملة + الاحتياط عند لغة غير معينة (تلف مخزن persist)، المسار الأصلي كاملاً: رقصة الأذونات بواجهتيها (الحديثة checkPermissions/requestPermissions والقديمة hasPermission/requestPermission) بمساراتها الست، مستمع partialResults، فشل المحرك (خطأ Error وغير Error)، تجاهل فشل removeAllListeners، عدم الإعادة عند الاستماع، stop.
  - **3. `settingsService.ts` (اكتشاف السجل الطازج — 408 أسطر عند 4.57% بلا أي اختبار مخصص، أعلى من كل بنود المقترح الأصلي عدا voiceAssistant): 4.57% → 94.1% عبارات، 89.3% دوال، 69.1% فروع** — 29 اختباراً (`tests/unit/settingsService.test.ts`) تثبّت حراسات أمن النسخ الاحتياطي: **رفض التصدير غير المشفر لخزنة محمية بـPIN** (بمفتاحي pinHash والقديم pin)، حد 6 رموز لكلمة مرور التشفير، تقشير أسرار الخزنة من التصدير حتى بلا PIN، **الحفاظ على هوية خزنة الجهاز عبر الاستعادة** (pinHash المحلي يبقى والأجنبي يُقلع)، استعادة localStorage بلا مفاتيح الأسرار، إعدادات dict القديمة، التراجع الذري عند فشل منتصف الكتابة (bulkPut يفشل ← المعاملة تتراجع ← البيانات الأصلية سليمة)، سجل التدقيق import_data، الاستدعاء عبر evaluateRestoreProtection، بذور البيانات التجريبية، المسح الشامل المؤكد، إعادة حساب الأرصدة (رصيد ابتدائي + دخل − مصروف، والمعاملات اليتيمة تُتجاهل).
  - **4. رفعة السقاطة الأولى الموثقة**: 60.0/56.5/45.0 → **62.0/57.5/46.5** (مقيس 62.21-62.26/57.68/46.65 عبر تشغيلين مستقلين) — تاريخ المعايرة الكامل داخل `vitest.config.ts`.
  - **5. البوابات**: **1000/1000 اختباراً × 110 أجنحة** · `tsc` 0 · `ci:check` كاملة exit 0 (تغطية فوق العتبة المرفوعة + فحص أمني نظيف + 0 ثغرات + lint نظيف + بناء نظيف 7.49s) · guardian نظيف.
  - **6. دروس بيئية موثقة للجولات القادمة**: mock الـlocalStorage في setup.ts لا يدعم length/key(i) (حلقة جمع البيانات لا ترى شيئاً — عولج بمحاكي كامل داخل ملف الاختبار)؛ `DB.tables` يعيد مصفوفة جديدة كل نداء لكن كائناته مستقرة (التجسس يجب أن يستهدف نسخة tables لا وصول الـaccessor).
  - **7. سلم التوجيه 18 المتبقي**: exportService 0.81% (ملف اختباره الشكلي لا يغطي شيئاً)، sabBanner 38.46%، مخازن envelope/family/preferences ~40% لكل منها، statementParser 73.55% (تصحيح اسمه — كان يدون سابقاً paymentParser)، marketData.ts 73%.
