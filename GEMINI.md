...
- **التوجيه 17 — البندان 2 و3 من طاولة المقترحات: بوابة التدقيق الأمني في CI + دلالة «أول نتيجة» لـisLoading (Directive 17: CI Security Gate & isLoading First-Result Semantics)**:
  - **0. سياق التفويض**: البند 1 (تفعيل Crashlytics) ما زال محجوزاً على `google-services.json` من المالك. البندان المتبقيان غير المحجوبين نُفِّذا كاملين أدناه.
  - **1. البند 2 — تدقيق التبعيات: الاكتشاف قبل الإصلاح**:
  ...
  - **6. بوابات الجودة**: **914/914 اختباراً بنسبة 100% عبر 106 أجنحة** (906 + 8، صفر انحدار — منها اختبارات advisorPage التسعة وfetchErrorDisambiguation كاملة) · `tsc --noEmit` صفر · `eslint` صفر · `guardian.mjs validate` نظيف · صفر مفاتيح توطين جديدة · `npm run audit:security` نظيف.
  - **7. المتبقي على طاولة التوجيه 17**: البند 1 فقط — تفعيل Crashlytics الأصلي (الخطوات الأربع الموثقة في `crashlytics.ts`)، محجوز على تزويد المالك بـ`google-services.json`.
